...
def record_delete(request, id, name):
    if request.method == 'POST':
        ...
        if record.editable_by(request.user):
            print 'record ='+ str(type(record)) + 'i am record: ' + str(record)
            storage, created = Storage.objects.get_or_create(name='unitedsearch')
            #print 'potential scratch dir: '+storage.get_derivative_storage_path()
            der_path = storage.get_derivative_storage_path()
            scratchlist = os.listdir(der_path)
            prefix = str(id)+'-'
            for der in scratchlist:
                if der.startswith(prefix):
                    print 'deleting temp file '+der
                    #storage.delete_file(der)
                    os.remove(der_path+'/'+der)
            storage_dir = storage.base
            image_file = record.get_image_url()
            print 'deleting record image ' + image_file
            record.delete()
