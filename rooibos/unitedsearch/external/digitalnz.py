...
documentation for the digitalNZ API can be found at 
    http://www.digitalnz.org/developers/api-docs-v3/search-records-api-v3
you can also find the getMetaData API at 
    http://www.digitalnz.org/developers/api-docs-v3/get-metadata-api-v3
Calls to the digitalnz API can return a json, rss or xml - this system uses 
    json and then loads it to a python object

DigitalNZ can handle modifiers and an empty keywords field, as long as there are
    other parameters to search by.
You will need to get your own API key and place it in settings_local.py as DNZ_API_KEY 
    the URL builder here will load it up
"""
import json
import urllib2
...
from rooibos.unitedsearch.common import modulate_offset 
...
def _build_URL(query, params, per_page, page):
    ...
    url = ""
    query_terms = params.copy() if params else _translate_query(query) 
        #if not params else parse_parameters(params)
